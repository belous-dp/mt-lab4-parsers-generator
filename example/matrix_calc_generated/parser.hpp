...
enum class token { MATRIX, INV, PLUS, MINUS, MMUL, MDIV, MUL, DIV, LP, RP, _END };
...
struct lexer {
  ...
  token next_token() {
    ...
    if (auto m = ctre::starts_with<"\\[\\[.*?\\]\\]">(input)) {
        t = token::MATRIX;
        take(m.size());
    } else if (auto m = ctre::starts_with<"\\^\\(-1\\)|\\^-1">(input)) {
        t = token::INV;
        take(m.size());
...
class parser {
  ...
  tp tp(matrix acc) {
    ...
    case token::MDIV: {
      ...
      auto _2 = f();
      _res.children.emplace_back(_2);
      auto _3 = tp(division(acc, _2.val));
      _res.children.emplace_back(_3);
      _res.val = _3.val;
      break;
    }
  ...
  f f() {
    ...
    case token::LP: {
      auto _1 = i();
      _res.children.emplace_back(_1);
      auto _2 = lexer.cur_token_val();
      _res.children.emplace_back(_2);
      lexer.next_token();
      _res.val = inverse(_1.val);
      break;
    }
    default:
